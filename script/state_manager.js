...
var StateManager = {
	...
	updateOldState: function(){
		...
		if(version == 1.3) {
			...
			Engine.log('upgraded save to v1.4 (Demon Slayer themed key rename)');
			$SM.set('version', 1.4);
		}
		// 幂等清洗：即使 version 已 >= 1.4，也再扫一次，兜底处理运行时残留的旧 key
		// （例如曾经的事件 reward 用了 bullets/rifle 字面，留在存档里的孤儿数据）
		$SM.cleanupRenamedKeys();
	},
	...
	cleanupRenamedKeys: function() {
		...
		for (var sK in STORE_RENAME) {
			...
			var oldBpS = sK + ' blueprint';
			var newBpS = STORE_RENAME[sK] + ' blueprint';
			if (Object.prototype.hasOwnProperty.call(stores, oldBpS)) {
				stores[newBpS] = (stores[newBpS] || 0) + (stores[oldBpS] || 0);
				delete stores[oldBpS];
				dirty = true;
			}
		}
		if (dirty) $SM.set('stores', stores, true);
		...
		for (var oK in STORE_RENAME) {
			...
			var oldBpO = oK + ' blueprint';
			var newBpO = STORE_RENAME[oK] + ' blueprint';
			if (Object.prototype.hasOwnProperty.call(outfit, oldBpO)) {
				outfit[newBpO] = (outfit[newBpO] || 0) + (outfit[oldBpO] || 0);
				delete outfit[oldBpO];
				dirty = true;
			}
		}
		if (dirty) $SM.set('outfit', outfit, true);

		dirty = false;
		var perks = $SM.get('character.perks') || {};
		for (var pK in PERK_RENAME) {
			if (perks[pK]) {
				perks[PERK_RENAME[pK]] = true;
				delete perks[pK];
				dirty = true;
			}
		}
		if (dirty) $SM.set('character.perks', perks, true);
		...
		if (dirty) $SM.set('character.blueprints', blueprints, true);
	},

	// 旧→新 key 映射表，供 updateOldState 与少数运行时 fallback 使用。
	_RENAME_STORES: {
		'alien alloy':     'demon stone',
		'energy cell':     'solar crystal',
		'bullets':         'wisteria bullet',
		'charm':           'wisteria charm',
		'bolas':           'kusarigama',
		'grenade':         'wisteria bomb',
		'bayonet':         'nichirin spear',
		'rifle':           'wisteria gun',
		'laser rifle':     'nichirin gun',
		'plasma rifle':    'thunder gun',
		...
		'glowstone':       'firefly orb',
		'cargo drone':     'cargo crow',
		'kinetic armour':  'wind armour',
		'fluid recycler':  'water cycle'
	},
	_RENAME_PERKS: {
		'boxer':           'fist form one',
		'martial artist':  'fist form four',
		'unarmed master':  'fist form master',
		'barbarian':       'slash mastery',
		'slow metabolism': 'breath no food',
		'desert rat':      'breath no water',
		'evasive':         'step yushin',
		'precise':         'mikiri',
		'scout':           'crow scout',
