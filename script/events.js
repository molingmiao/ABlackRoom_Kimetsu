...
var Events = {
	...
	_MERCHANT_INTERVAL: 10 * 60 * 1000, // 游方铁匠出现间隔（10 分钟）
	_PANEL_FADE: 200,
	_FIGHT_SPEED: 100,
	_EAT_COOLDOWN: 5,
	_MEDS_COOLDOWN: 7,
	...
	// 游方铁匠：每 _MERCHANT_INTERVAL 尝试出现一次，检查条件后触发 MerchantScrap
	tryMerchant: function() {
		if (typeof Room === 'undefined' || typeof Events.MerchantScrap === 'undefined') return;
		if (Engine.activeModule !== Room) return;
		if (Events.activeEvent()) return;
		if (!$SM.get('features.location.outside')) return;
		var hasWeapon = false;
		for (var key in Room.Craftables) {
			if (Room.Craftables[key].type === 'weapon' && $SM.get('stores["' + key + '"]', true) > 0) {
				hasWeapon = true;
				break;
			}
		}
		if (!hasWeapon) return;
		Events.startEvent(Events.MerchantScrap);
	},

	startMerchantTimer: function() {
		if (Events._merchantTimer) clearInterval(Events._merchantTimer);
		Events._merchantTimer = setInterval(Events.tryMerchant, Events._MERCHANT_INTERVAL);
	},
	...
	startCombat: function(scene) {
		...
		*/
		...
		if ((Path.outfit['concentration pill'] ?? 0) > 0) {
		...
		Events._specialTimers = (scene.specials ?? []).map(s => Engine.combatSetInterval(
			() => {
				const enemy = $('#enemy');
	...
	startEnemyAttacks: (delay) => {
		...
		Events._enemyAttackTimer = Engine.combatSetInterval(Events.enemyAttack, (delay ?? scene.attackDelay) * 1000);
	},
	...
	createAttackButton: function(weaponName) {
		...
		if(weapon.type == 'unarmed') {
			if($SM.hasPerk('fist form master')) {
				cd /= 2;
			}
		}
		var btn = new Button.Button({
		...
		// 按武器类型添加识别 class，在 CSS 中以不同左边框颜色区分
		if (weapon.type === 'melee') btn.addClass('btnMelee');
		else if (weapon.type === 'ranged') {
			if (weapon.damage === 'stun') btn.addClass('btnStun');
			else btn.addClass('btnRanged');
		}
		else if (weapon.type === 'unarmed') btn.addClass('btnUnarmed');
	...
	useHypo: btn => {
		Events.doHeal('wisteria oil', World.hypoHeal(), btn);
		AudioEngine.playSound(AudioLibrary.USE_MEDS);
	},
	...
	useWeapon: function(btn) {
		if(Events.activeEvent()) {
			...
			if(weapon.type == 'unarmed') {
				...
				if($SM.get('character.punches') == 50 && !$SM.hasPerk('fist form one')) {
					$SM.addPerk('fist form one');
				} else if($SM.get('character.punches') == 150 && !$SM.hasPerk('fist form four')) {
					$SM.addPerk('fist form four');
				} else if($SM.get('character.punches') == 300 && !$SM.hasPerk('fist form master')) {
					$SM.addPerk('fist form master');
				}

			}
			// 武器熟练度：按武器名计数，达阈解锁对应呼吸 perk
			var weaponHitKey = 'character.weaponHits["' + weaponName + '"]';
			if (!$SM.get(weaponHitKey)) $SM.set(weaponHitKey, 0);
			$SM.add(weaponHitKey, 1);
			var hits = $SM.get(weaponHitKey);
			if (weaponName === 'nichirin katana' && hits === 100 && !$SM.hasPerk('water breath I')) {
				$SM.addPerk('water breath I');
			} else if (weaponName === 'wisteria gun' && hits === 50 && !$SM.hasPerk('flame breath I')) {
				$SM.addPerk('flame breath I');
			} else if (weaponName === 'thunder gun' && hits === 30 && !$SM.hasPerk('thunder breath I')) {
				$SM.addPerk('thunder breath I');
			}
			if(weapon.cost) {
				var mod = {};
				var out = false;
				for(var k in weapon.cost) {
					if(typeof Path.outfit[k] != 'number' || Path.outfit[k] < weapon.cost[k]) {
						if (!Engine.options.testerMode) return;
					}
					mod[k] = -weapon.cost[k];
			...
			if(Math.random() <= World.getHitChance()) {
				...
				if(typeof dmg == 'number') {
					...
					if(weapon.type == 'unarmed' && $SM.hasPerk('fist form four')) {
						dmg *= 3;
					}
					if(weapon.type == 'unarmed' && $SM.hasPerk('fist form master')) {
						dmg *= 2;
					}
					// 跨周目 buff：拳脚伤害叠加
					if (weapon.type == 'unarmed') {
						try { dmg += Prestige.getBonus('fistDmg') || 0; } catch (e) { /* ignore */ }
					}
					if(weapon.type == 'melee' && $SM.hasPerk('slash mastery')) {
						dmg = Math.floor(dmg * 1.5);
					}
					// 呼吸之式：武器熟练度解锁的被动 +1 伤害（可叠加）
					if (weapon.type == 'melee' && $SM.hasPerk('water breath I')) {
						dmg += 1;
					}
					if (weapon.type == 'ranged' && $SM.hasPerk('flame breath I')) {
						dmg += 1;
					}
					if (weapon.type == 'ranged' && $SM.hasPerk('thunder breath I')) {
						dmg += 1;
					}
					// 日轮刀颜色 buff：仅 'nichirin katana' 触发，颜色由当前 perks 派生
					if(weaponName === 'nichirin katana' && Engine.NichirinColors) {
		...
		if(typeof dmg == 'number') {
			if(dmg < 0) {
				...
				if (venomous && !shielded) {
					...
					Events._dotTimer = Engine.combatSetInterval(() => {
	...
	enemyAttack: function() {
		...
		if(!stunned && !meditating) {
			var toHit = scene.hit;
			toHit *= $SM.hasPerk('step yushin') ? 0.8 : 1;
			var dmg = -1;
	...
	checkPlayerDeath: () => {
		if($('#wanderer').data('hp') <= 0) {
			// 鬼灭标记 / Total Concentration Breathing: one-time auto-recovery in combat
			if (!Events._deathRecoveryUsed && $SM.hasPerk('total concentration')) {
				Events._deathRecoveryUsed = true;
	...
	winFight: function() {
		Engine.setTimeout(function() {
			...
			// 成就计数：某些 boss 击杀记事件状态
			try {
				var curScene = Events.activeEvent() && Events.activeEvent().scenes[Events.activeScene];
				var foe = (curScene && (curScene.enemy || '')) + '';
				if (foe === 'upper moon candidate') {
					$SM.add('game.upperMoonSlain', 1);
				}
			} catch (e) { /* ignore */ }
	...
	drawLoot: function(lootList) {
		...
		// 跨周目 buff：事件 loot 倍率
		var lootMult = 1;
		try { lootMult = 1 + (Prestige.getBonus('eventLoot') || 0); } catch (e) { /* ignore */ }
