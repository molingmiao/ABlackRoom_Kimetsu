...
var Outside = {
	...
	init: function(options) {
		...
		if ($SM.get('features.location.autoActions')) {
			Outside.ensureAutoActionsBtn();
		}
		// 存档里记录开启且已解锁，重启自动恢复
		if ($SM.get('features.location.autoActions') && $SM.get('config.autoActions', true)) {
			Outside.startAutoActions();
		}
	},

	// 自动伐木/查陷阱：人口首次达到上限后永久解锁，可 toggle
	autoActionsInterval: null,

	startAutoActions: function() {
		if (Outside.autoActionsInterval) return;
		if (!$SM.get('features.location.autoActions')) return;
		Outside.autoActionsInterval = setInterval(function() {
			if (Engine.activeModule !== Outside) return;
			$('#gatherButton').not('.disabled').click();
			$('#trapsButton').not('.disabled').click();
		}, 1000);
	},

	stopAutoActions: function() {
		if (Outside.autoActionsInterval) {
			clearInterval(Outside.autoActionsInterval);
			Outside.autoActionsInterval = null;
		}
	},

	toggleAutoActions: function() {
		if (!$SM.get('features.location.autoActions')) return;
		var on = $SM.get('config.autoActions', true);
		var next = !on;
		$SM.set('config.autoActions', next, true);
		if (next) Outside.startAutoActions();
		else Outside.stopAutoActions();
		Outside.updateAutoActionsBtn();
	},

	ensureAutoActionsBtn: function() {
		if ($('#autoActionsBtn').length > 0) return;
		new Button.Button({
			id: 'autoActionsBtn',
			text: _('auto: off'),
			click: Outside.toggleAutoActions,
			width: '80px'
		}).appendTo('div#outsidePanel');
	},

	updateAutoActionsBtn: function() {
		var maxPop = Outside.getMaxPopulation();
		var hasCap = maxPop > 0;
		var everUnlocked = !!$SM.get('features.location.autoActions');

		// 首次拥有人口容量（建了第一间小屋）：永久解锁并醒目提示
		if (hasCap && !everUnlocked) {
			$SM.set('features.location.autoActions', true);
			everUnlocked = true;
			Notifications.notify(Outside, _('永久增益：搭起小屋后自动化解锁。左侧「自动：开/关」按钮可让村民自动伐木/查陷阱。'));
		}

		if (!everUnlocked) return; // 从未解锁：不显示按钮

		Outside.ensureAutoActionsBtn();
		var btn = $('#autoActionsBtn');
		var on = $SM.get('config.autoActions', true);
		Button.setDisabled(btn, false);
		btn.text(on ? _('auto: on') : _('auto: off'));

		// 首次解锁时高亮，玩家点击一次后自动消除
		if (!$SM.get('game.knownButtons["autoActions"]')) {
			btn.addClass('newly-unlocked');
			btn.off('click.unlocked').one('click.unlocked', function() {
				$(this).removeClass('newly-unlocked');
				$SM.set('game.knownButtons["autoActions"]', true, true);
			});
		}
	},
	...
	gatherWood: function() {
		...
		// 跨周目 buff：伐木倍率
		try {
			var mult = 1 + (Prestige.getBonus('woodGather') || 0);
			if (mult > 1) gatherAmt = Math.max(gatherAmt, Math.floor(gatherAmt * mult));
		} catch (e) { /* ignore */ }
		$SM.add('stores.wood', gatherAmt);
		// 记录伐木次数（成就计算用）
		$SM.add('game.gatherCount', 1);
	...
	checkTraps: function() {
		...
		drops['bait'] = -baitUsed;
		// 跨周目 buff：陷阱产出倍率（仅作用于正收获项，bait 除外）
		try {
			var trapMult = 1 + (Prestige.getBonus('trapDrop') || 0);
			if (trapMult > 1) {
				for (var dn in drops) {
					if (dn === 'bait') continue;
					if (drops[dn] > 0) drops[dn] = Math.max(1, Math.floor(drops[dn] * trapMult));
				}
			}
		} catch (e) { /* ignore */ }
		// 记录查陷阱次数
