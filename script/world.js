var World = {
  ...
  useSupplies: function() {
    ...
    movesPerWater *= $SM.hasPerk('breath no water') ? 2 : 1;
  ...
  meatHeal: function() {
    return World.MEAT_HEAL * ($SM.hasPerk('breath nourish') ? 2 : 1);
  },
  ...
  checkFight: function() {
    ...
    if(World.fightMove > World.FIGHT_DELAY) {
      ...
      chance *= $SM.hasPerk('kehai dansha') ? 0.5 : 1;
      // 紫藤护身符减少遭遇：每个 -10%，最低乘到 0.3。若仍触发遭遇，消耗 1 个
      var charms = (Path.outfit && Path.outfit['wisteria charm']) || 0;
      if (charms > 0) {
        chance *= Math.max(0.3, 1 - 0.1 * charms);
      }
      if(Math.random() < chance) {
        World.fightMove = 0;
        if (charms > 0) {
          Path.outfit['wisteria charm'] = charms - 1;
          $SM.set('outfit["wisteria charm"]', Path.outfit['wisteria charm']);
          try { Notifications.notify(null, _('a wisteria charm crumbles to dust as a demon nears.')); } catch (e) { /* ignore */ }
          try { World.updateSupplies(); } catch (e) { /* ignore */ }
        }
        Events.triggerFight();
      }
    }
  ...
  redeemBlueprints: () => {
    ...
    redeem('wisteria oil blueprint', 'wisteria oil');
    redeem('wind armour blueprint', 'wind armour');
    redeem('bind kunai blueprint', 'bind kunai');
    redeem('thunder gun blueprint', 'thunder gun');
    redeem('concentration pill blueprint', 'concentration pill');
  ...
  _reportExpeditionSummary: function(isDeath) {
    ...
    var consumed = {};
    var gained = {};
    var outfitKeys = new Set(Object.keys(beforeOutfit));
    Object.keys(afterOutfit).forEach(function(k) { outfitKeys.add(k); });
    ...
    var fmt = function(dict) {
      ...
      return keys.map(function(k) { return _(k) + ' ×' + dict[k]; }).join('、');
    };
    var consumedStr = fmt(consumed);
    var gainedStr = fmt(gained);
    var parts = [];
  ...
  leaveItAtHome: function(thing) {
     return thing != 'cured meat' && thing != 'wisteria bullet' && thing != 'solar crystal'  && 
      thing != 'wisteria charm' && thing != 'medicine' && thing != 'concentration pill' && thing != 'wisteria oil' &&
  ...
  getMaxWater: function() {
    ...
    if($SM.get('stores["water cycle"]', true) > 0) {
